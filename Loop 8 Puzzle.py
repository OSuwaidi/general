...
def match(parent, target):
    ...
    while True:
        ...
        if c < 2:
            ...
            if not (current == visited).all((1, 2)).any():  # "axis=1" is comparing columns, "axis=2" is comparing rows, ***"axis=(1,2)" is comparing both rows and columns (compares entire matrices)***
        ...
        if c > 0:
            current[r, c] = current[r, c - 1]
            current[r, c - 1] = temp[r, c]
            if not (current == visited).all((1, 2)).any():
        ...
        parent = visited[n]  # Starts with n=1, since n=0 is the initial state we started with ("start")
