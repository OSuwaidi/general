...
y_b = 0
y_r = 1
alpha = 0.1  # The learning rate is the MOST IMPORTANT hyperparamter to tune when training a neural network (notice that we increased initial alpha to 1.4 for it to converge faster at the beginning, but then toned it down a bit, so that it won't cause the weights to bounce-off/oscillate back and forth around the minimum of the cost surface/curvature)
...
cost = 0
properties = {1: {'color': 'r', 'label': 'Red Flower'}, 0: {'color': 'b', 'label': 'Blue Flower'}}

# Construct a scatter plot of the decision space:
for sample in data:
    plt.scatter(sample[0], sample[1], c=properties[sample[2]]['color'])
...
for sample in data:
    cost += sympy.exp((sigmoid(wh*(w1*sample[0] + w2*sample[1] + b)) - sample[2])**2) - 1  # Here we are taking the cost of ALL our samples, in practice you would divide the number of samples into "batches", and then pass each batch into the cost to process gradient descent, then repeat for all the different batches till you complete the entire sample set (cover all the batches = 1 epoch)
print(f"cost = {cost} \n")
...
for i in range(500):  # Notice: we decreased the number of epochs because we were able to increase our learning parameter instead
    w1 -= alpha * dw1(w1, w2, b, wh)
    w2 -= alpha * dw2(w1, w2, b, wh)
    b -= alpha * db(w1, w2, b, wh)
    wh -= alpha * dwh(w1, w2, b, wh)
...
for i in data:
    ...
    space.scatter(i[0], i[1], sigmoid(wh*(w1*i[0] + w2*i[1] + b)), s=100, c=color)  # "a.scatter3D(x, y, z, size=100, color='r')"
